...
const ProjectDetailsPage: React.FC = () => {
    ...
    if (!project) {
        return (
            <div className="min-h-screen bg-dark-300 flex items-center justify-center">
                <p className="text-white text-2xl font-display">Proyek Tidak Ditemukan...</p>
                <Link to="/" className="ml-4 text-primary-500 hover:underline">Kembali</Link>
            </div>
        );
    }
    ...
    return (
        <div className="min-h-screen bg-dark-300 overflow-x-hidden pt-12">
            <Helmet>
                <title>{project.name} - Fauzi Hartanto</title>
                <meta name="description" content={`Studi kasus mendalam dan deskripsi detail proyek ${project.name}.`} />
                <link rel="canonical" href={`https://github.com/FauziHrtnt/project/${project.id || id}`} />
            </Helmet>
            ...
            <div ref={containerRef} className="container mx-auto px-4 max-w-6xl py-12">
                ...
                <header className="mb-16">
                    <span className="header-fade inline-block px-3 py-1.5 rounded-lg bg-primary-600/20 text-primary-400 border border-primary-500/30 text-sm font-bold uppercase tracking-wider mb-6">
                        {project.type === 'ANDROID_APP' || project.type === 'android' ? 'Android Native' : (project.type === 'Websites' || project.type === 'web' ? 'Situs & Aplikasi Web' : project.type.replace('_', ' '))}
                    </span>
                ...
                <div className="content-fade grid grid-cols-1 lg:grid-cols-3 gap-12 mb-20">
                    ...
                    <div className="lg:col-span-1 space-y-12">
                        ...
                        <div className="p-8 bg-dark-200/50 rounded-3xl border border-gray-800">
                            <h3 className="text-xl font-display font-bold text-white mb-6 flex items-center gap-2">
                                <Layers className="text-primary-500" size={24} /> Teknologi
                            </h3>
                        ...
                        <div className="p-8 bg-dark-200/50 rounded-3xl border border-gray-800">
                            <h3 className="text-xl font-display font-bold text-white mb-6 flex items-center gap-2">
                                <Globe className="text-primary-500" size={24} /> Tautan
                            </h3>
                            <div className="flex flex-col gap-4">
                                {project.link && (
                                    <a href={project.link} target="_blank" rel="noopener noreferrer" className="flex items-center gap-3 p-4 bg-dark-100 hover:bg-white hover:text-dark-300 text-gray-300 rounded-xl transition-colors font-medium">
                                        ...
                                        {project.link.includes('github.com') ? 'Lihat Source Code' : 'Kunjungi Proyek'}
                                    </a>
                                )}
                                {!project.link && (
                                    <p className="text-gray-500 italic">Tautan proyek tidak tersedia secara publik.</p>
                                )}
                            </div>
                        </div>
                    </div>
                    ...
                    <div className="lg:col-span-2 space-y-12">
                        <section className="prose prose-invert prose-lg max-w-none">
                            <h2 className="text-3xl font-display font-bold text-white mb-6">Tentang Proyek</h2>
                ...
                {project.images && project.images.length > 0 && (
                    <section className="content-fade mb-20">
                        <h2 className="text-3xl font-display font-bold text-white mb-8">Galeri Screenshot</h2>
                        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-8">
                            {project.images.map((img: string, index: number) => (
                                <div key={index} className={`bg-dark-200 border border-gray-800 rounded-3xl overflow-hidden flex items-center justify-center relative group p-4 xl:p-6 transition-all duration-300 hover:border-primary-500/50 hover:-translate-y-2 ${(project.type === 'Websites' || project.type === 'web') ? 'aspect-video' : 'aspect-[9/16]'}`}>
